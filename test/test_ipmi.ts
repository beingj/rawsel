...
describe("ipmi", () => {
    ...
    it("p_edf", () => {
        const et_ed1: Array<[number, number]> = [
        ...
        // threshold
        expect(p_edf(...et_ed1[0])).to.equal(
            "trigger reading in byte 2, trigger threshold value in byte 3"
        )
        // discrete: 0, 4, ah, fh
        expect(p_edf(...et_ed1[1])).to.equal(
            "unspecified byte 2, unspecified byte 3"
        )
        expect(p_edf(...et_ed1[2])).to.equal(
            "previous state and/or severity in byte 2, unspecified byte 3"
        )
        expect(p_edf(...et_ed1[3])).to.equal(
            "OEM code in byte 2, OEM code in byte 3"
        )
        expect(p_edf(...et_ed1[4])).to.equal(
            "sensor-specific event extension code in byte 2, sensor-specific event extension code in byte 3"
        )
        // sensor-specific: 0, 4, ah, fh
        expect(p_edf(...et_ed1[5])).to.equal(
            "unspecified byte 2, unspecified byte 3"
        )
        expect(p_edf(...et_ed1[6])).to.equal(
            "previous state and/or severity in byte 2, unspecified byte 3"
        )
        expect(p_edf(...et_ed1[7])).to.equal(
            "OEM code in byte 2, OEM code in byte 3"
        )
        expect(p_edf(...et_ed1[8])).to.equal(
            "sensor-specific event extension code in byte 2, sensor-specific event extension code in byte 3"
        )
        // OEM: 0, 4, ah, fh
        expect(p_edf(...et_ed1[9])).to.equal(
            "unspecified byte 2, unspecified byte 3"
        )
        expect(p_edf(...et_ed1[10])).to.equal(
            "previous state and/or severity in byte 2, unspecified byte 3"
        )
        expect(p_edf(...et_ed1[11])).to.equal(
            "OEM code in byte 2, OEM code in byte 3"
        )
