export class IPMI_Spec {
    ...
    static event_data: { [sensor_type: number]: { [offset: number]: (selr: { event_data2: number, event_data3: number }) => { d2?: string, d3?: string } } } = {
        0x05: {
            // Physical Security
            0x04: (selr) => {
                // LAN Leash Lost
        ...
        0x28: {
            ...
            0x05: (selr) => {
                ...
                const d2 = `device is ${isLogic ? '' : 'not'} a logical FRU Device, \
LUN ${(ed2 >> 3) & 0x3}, bus ID ${ed2 & 3}`
                let d3: string
                if (isLogic) {
                    d3 = 'FRU Device ID:'
                } else {
...
// memory offset 0~8h
for (let i = 1; i <= 8; i++) {
    IPMI_Spec.event_data[0x0c][i] = IPMI_Spec.event_data[0x0c][0x00]
}
// System Firmware Hang
IPMI_Spec.event_data[0x0f][0x01] = IPMI_Spec.event_data[0x0f][0x00]
// slot/connector offset 0~9h
for (let i = 1; i <= 9; i++) {
    IPMI_Spec.event_data[0x21][i] = IPMI_Spec.event_data[0x21][0x00]
}
// watchdog 2 offset 0~3, 8h
for (let i = 1; i <= 3; i++) {
    IPMI_Spec.event_data[0x23][i] = IPMI_Spec.event_data[0x23][0x00]
}
IPMI_Spec.event_data[0x23][8] = IPMI_Spec.event_data[0x23][0x00]
