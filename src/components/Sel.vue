<template>
  <v-container fluid>
    ...
    <div @drop.prevent="add_drop_file" @dragover.prevent>
      <v-file-input show-size dense label="raw SEL file" multiple chips v-model="files"></v-file-input>
    </div>
    <v-data-table
      :headers="headers"
      :items="rows"
...
export default Vue.extend({
  ...
  methods: {
    ...
    update_sel_with_sdr(sels: SelRecord[], sdrs: SdrRecord[], rows: sel_row[]) {
      // could be type1/2/3, but use type1 for short
      const sdr_has_sensor_num = sdrs.filter((i: SdrRecord) =>
        Object.keys(i).indexOf('sensor_num') >= 0) as SdrRecordType1[]
