...
`
        ...
        expect(sdr.sdr_version).to.equal(0x51)
        expect(sdr.record_type).to.equal(
            SdrRecordType.ManagementControllerDeviceLocator
        )
        expect(sdr.record_length).to.equal(0x12)
    })
...
`
        ...
        expect(sdr.reading(100)).to.equal(
            Math.pow(Math.pow(60, 3), -1).toFixed2(2)
        )
...
`
        ...
        expect(sdr.get_reading_formula_text_full()).to.equal(
            `(1x + (0 \\\\times 10 ^ {0})) \\\\times 10 ^ {0}`
        )
    })
    it("formula mathjax", () => {
...
`
        ...
        expect(sdr.get_reading_formula_text()).to.equal(
            `$$x \\times 10 ^ {2}$$`
        )
        sdr.m = 1
        sdr.b = 0
        ...
        // (2x + ((-2) \\times 10 ^ {0})) \\times 10 ^ {2}
        expect(sdr.get_reading_formula_text()).to.equal(
            `$$${name_of_linear(sdr.linear)}[(2x + (-2)) \\times 10 ^ {2}]$$`
        )
    })
    it("threshold", () => {
        const hex = `
...
`
        ...
        expect(sdr.event?.map((i) => i.s).join(",")).to.equal(
...
`
        const dv = new DataView(hex2ArrayBuffer(hex, 4))
        dv.setUint8(13, EventType.threshold)
        // https://stackoverflow.com/a/22340179
        expect(() => new SdrRecordType2(dv)).to.throw(
